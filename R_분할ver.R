...
DB <- paste0("select ctc.voc_id , ctc.voc_dt , coo.delivery_expected_date ,
...
order_date <- dbGetQuery(redshift_conn, "select cog.group_order_no, cog.payment_completed_at

                                              from commerce_order.group_order as cog") %>%

  set_utf8() %>% tbl_df() %>% filter(!is.na(group_order_no) & !is.na(payment_completed_at))

 

voc_today <- DB1 %>% left_join(order_date) %>% filter(!is.na(group_order_no))

 

 

voc_today $payment_completed_at <- as.Date(voc_today$payment_completed_at)

voc_today $group_order_no <- as.Date(voc_today$group_order_no)

 

 

suply_date <- dbGetQuery(redshift_conn, "select sms.sply_cd , sms.sply_nm , sms.md, sms.master_cd

                                         from mkrs_aa_schema.u_md_sply_mthly_1d as sms") %>%

  set_utf8() %>% tbl_df() %>% filter(!is.na(sply_nm) & !is.na(sply_cd))

 

voc_today2 <- voc_today %>% left_join(suply_date, by=c('master_cd' = 'goods_cd')) %>% filter(!is.na(master_cd))

 

 

voc_today2$sply_cd <- as.character(voc_today2$sply_cd)

voc_today2$sply_nm <- as.character(voc_today2$sply_nm)

voc_today2$md <- as.character(voc_today2$md)

 

 

category_date <- dbGetQuery(redshift_conn, "select spi.deal_nm , spi.catg_2_nm , spi.catg_3_nm, spi.catg_1_cd , spi.catg_2_cd

                                            from mkrs_aa_schema.prd_info_1d as spi
...
voc_today3 <- voc_today2 %>% left_join(category_date, by=c('master_cd' = 'goods_cd')) %>% filter(!is.na(master_cd))

 

 

voc_today3$deal_nm <- as.character(voc_today3$deal_nm)

voc_today3$catg_2_nm <- as.character(voc_today3$catg_2_nm)

voc_today3$catg_3_nm <- as.character(voc_today3$catg_3_nm)


 

voc_today4 <- arrange(voc_today3,desc(voc_dt), voc_id , payment_completed_at , delivery_expected_date , call_class_nm , 
...
voc_today5 <- voc_today4 %>% distinct(order_no, goods_cd, .keep_all=T)
...
voc_today_keyword_real <- 
  ...
  voc_today5 %>% filter(call_cat_nm3 %in% c('냄새', '색', '크기/모양', '중량/수량', '상해/피부이상', '구성품 누락', '비식품 성능/작동/조립', '컬리품질보증제', 
